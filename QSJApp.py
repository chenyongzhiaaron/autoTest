...
log_file = os.path.join(os.getcwd(), 'log/qsj_app_api.log')
...
def runTest(testCaseFile):
    ...
    testCaseFile = testCaseFile.replace('/', '\\')
    print(testCaseFile)
...
def interfaceTest(num, api_purpose, api_host, request_url, request_data, check_point, request_method, request_data_type,
    ...
    if request_method == 'POST':
        conn.request('POST', request_url, str(request_data), headers=headers)
    elif request_method == 'GET':
        conn.request('GET', request_url + '?' + str(request_data), headers=headers)
    else:
...
# 发送通知邮件
def sendMail(text):
    sender = 'kira@sinaif.com'
    receiver = ['kira@sinaif.com']  # 接收邮件，可设置为你的QQ邮箱或者其他邮箱 'xxxx@sinaif.com'
    mailToCc = ['kira@sinaif.com']
    subject = '[AutomantionTest]接口自动化测试报告通知'
    smtpserver = 'smtp.sinaif.com'  # 设置服务器
    username = 'kira@sinaif.com'  # 用户名
    password = 'ankani#13'  # 密码

    # 三个参数：第一个为文本内容，第二个设置文本格式，第三个设置编码
    msg = MIMEText(text, 'html', 'utf-8')
    msg['Subject'] = subject
    msg['From'] = sender  # 发送者
    msg['To'] = ';'.join(receiver, )  # 接受者
...
def main():
    ...
    if len(errorTest) >= 0:
        html = '<html><body>接口自动化定期扫描，共有 ' + str(len(
            errorTest)) + ' 个异常接口，列表如下：' + '</p><table><tr><th style="width:100px;">接口</th><th style="width:50px;">状态</th><th style="width:300px;">接口地址</th><th style="width:300px;">请求参数</th><th>接口返回值</th></tr>'
